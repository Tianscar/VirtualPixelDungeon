...
public class Random {
    ...
    public static long seed(){
       return seed = new Random().nextLong();
    }

	public static float Float( float min, float max ) {
		return (float)(min + new Random(seed()).nextDouble() * (max - min)); 
	}
	
	public static float Float( float max ) {
		return (float)(new Random(seed()).nextDouble() * max);
	}
	...
	public static int Int( int max ) {
		return max > 0 ? (int)(new Random(seed()).nextDouble() * max) : 0;
	}
	
	public static int Int( int min, int max ) {
		return min + (int)(new Random(seed()).nextDouble() * (max - min));
	}
	
	public static int IntRange( int min, int max ) {
		return min + (int)(new Random(seed()).nextDouble() * (max - min + 1));
	}
	
	public static int NormalIntRange( int min, int max ) {
		return min + (int)((new Random(seed()).nextDouble() + new Random(seed()).nextDouble()) * (max - min + 1) / 2f);
	}
	...
	public static int index( Collection<?> collection ) {
		return (int)(new Random(seed()).nextDouble() * collection.size());
	}
	
	public static<T> T oneOf( T... array ) {
		return array[(int)(new Random(seed()).nextDouble() * array.length)];
	}
	...
	public static<T> T element( T[] array, int max ) {
		return array[(int)(new Random(seed()).nextDouble() * max)];
	}

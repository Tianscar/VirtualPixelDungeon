...
public class WndBag extends WndTabbed {
    ...
    private Component lComponent = new Component();
...
//        list = new ScrollPane( lComponent );
//        add( list );
//        list.setRect( 0, txtTitle.height(), slotsWidth, slotsHeight + TITLE_HEIGHT );
//		
...
//		lComponent.add( new ItemButton( item ).setPos( x, y ) );
//        int slotsWidth = SLOT_SIZE * nCols + SLOT_MARGIN * (nCols - 1);
//		int slotsHeight = SLOT_SIZE * nRows + SLOT_MARGIN * (nRows - 1);
//		list.setSize( slotsWidth, slotsHeight + TITLE_HEIGHT );
		if (++col >= nCols) {
			col = 0;
			row++;
		}
